...
def view_store():
    ...
    if request.method == 'GET':
        ...
        if q:
            ...
            return render_template("view_store.html", products=results, product_image=images, q=q)
        else:
            products = AffiliateProduct.query.order_by(AffiliateProduct.id.desc()).paginate(page, per_page, error_out=False)
            return render_template("view_store.html", products=products, product_image=images, q=None)
    else:
        products = AffiliateProduct.query.order_by(AffiliateProduct.id.desc()).paginate(page, per_page, error_out=False)
        return render_template("view_store.html", products=products, product_image=images, q=None)
